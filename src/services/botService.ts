    ...
    if (pending.type === "email") {
      ...
      try {
        ...
        if (b?.success) {
          ...
          const existingCard = await Card.findOne({ userId: String(msg.chat.id) }).lean();
          if (existingCard) {
...
async function handleCardRequest(chatId: number, message?: any) {
  ...
  const existingCard = await Card.findOne({ userId }).lean();
...
async function refreshKycStatusFromStroWallet(user: any): Promise<KycStatus | undefined> {
  try {
    const existingCustomer = await Customer.findOne({ userId: String(user?.userId) }).lean();

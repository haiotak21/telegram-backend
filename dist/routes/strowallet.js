...
async function syncUserUsdtDepositsFromProvider(userId, address, limit = 20) {
    if (!address)
        return 0;
    ...
    for (const rawItem of items) {
        ...
        if ((0, persistence_1.isPrismaPersistenceEnabled)()) {
            ...
            try {
                ...
                await (0, botService_1.notifyDepositCredited)(userId, parsed.amount, nextBalance).catch(() => { });
                creditedCount += 1;
            }
            catch (e) {
        ...
        const updated = await User_1.default.findOneAndUpdate({ userId }, { $inc: { balance: parsed.amount } }, { new: true }).lean();
        await (0, botService_1.notifyDepositCredited)(userId, parsed.amount, Number(updated?.balance ?? 0)).catch(() => { });
        creditedCount += 1;
    }

...
router.post("/validate-transaction", async (req, res) => {
    try {
        ...
        const result = method === "telebirr" ? await (0, paymentServiceLegacy_1.validateTelebirrTransaction)(txn) : await (0, paymentServiceLegacy_1.validateCBETransaction)(txn);
        if (result.success)
            return (0, apiResponse_1.ok)(res, result);
        return (0, apiResponse_1.fail)(res, result.message || "Validation failed", 400);
    }
    catch (err) {

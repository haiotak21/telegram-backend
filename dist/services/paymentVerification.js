...
async function verifyPayment(input) {
    ...
    const { reference: cbeReference, suffixOverride } = paymentMethod === "cbe" ? parseCbeComposite(normalizedTxn) : { reference: normalizedTxn };
...
function getVerifierApiBase() {
    return (process.env.VERIFY_API_BASE ||
        process.env.PAYMENT_VALIDATION_BASE_URL ||
        "https://verifyapi.leulzenebe.pro").trim();
}
...
function shouldFallbackToCustomCbe(result) {
    ...
    return (message.includes("no pdf") ||
        message.includes("puppeteer") ||
        message.includes("pdf detected") ||
        message.includes("navigation"));
}
function shouldFallbackToStandaloneTelebirr(result) {
    if (result.body.success)
        return false;
...
async function verifyViaVerifierApi(params) {
    ...
    try {
        const response = await axios_1.default.post(`${base}${endpoint}`, payload, {
...
function normalizeVerifierApiResponse(data, transactionNumber, provider) {
    const success = Boolean(data?.success);
    if (!success) {
        return {
            status: 400,
    ...
    const amount = asNumberValue(details?.amount) ??
